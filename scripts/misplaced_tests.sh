...
# Collect all src directories from workspace crates
for crate in "${CRATES[@]}"; do
  crate_dir="$PROJECT_ROOT/$crate"
  if [ -d "$crate_dir/src" ]; then
    SRC_DIRS+=("$crate_dir/src")
  fi
done

# Check if we found any src directories
...
# Process each Rust file from all src directories
...
if [ "$files_with_tests" -gt 0 ]; then
  ...
  echo ""
  echo "All workspace crates checked:"
  for crate in "${CRATES[@]}"; do
    if [ -d "$PROJECT_ROOT/$crate/src" ]; then
      echo "  - $crate"
    fi
  done
fi
...
if [ "$files_with_tests" -gt 0 ]; then
  ...
  echo ""
  echo "⚠️  Tests should be moved to the tests/ directory"
  exit 1
else
  echo "Summary: All tests are correctly placed"
  exit 0
fi
echo "================================================================================"
